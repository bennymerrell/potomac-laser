...
// post, plus Elementor-built `page` drafts made by the page-sync pipeline (stamped
// _pl_auto_page). Live pages carry no such stamp, so they are unaffected. (2026-09-24)
if ( ! function_exists( 'pl_is_fullbleed_target' ) ) {
    function pl_is_fullbleed_target() {
        ...
        return is_singular( 'page' ) && get_post_meta( $id, '_pl_auto_page', true ) !== '';
    }
}
...
add_action( 'wp_head', function () {
    ...
    if ( $is_cls ) {
        echo '.cnc-elementor-fullbleed{overflow-x:hidden;}'
            ...
            . 'body:not(.page).cnc-elementor-fullbleed .e-con-inner{max-width:1224px!important;}'
            // pipeline page drafts: hide the page template's cover title; keep each section's own boxed_width
